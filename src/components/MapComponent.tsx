...
const MapComponent: React.FC<MapComponentProps> = ({
  ...
  // If admin is viewing all vehicles, center on first vehicle or default
  let allVehicleLatLngs: [number, number][] = [];
  if (vehicles && vehicles.length > 0) {
    allVehicleLatLngs = vehicles
      .map(v => v.location && v.location.latitude && v.location.longitude ? [v.location.latitude, v.location.longitude] : null)
      .filter(Boolean) as [number, number][];
  }
  const center = allVehicleLatLngs.length > 0
    ? allVehicleLatLngs[0]
    ...
    <div className="w-full h-full flex flex-col">
      <div className="relative flex-grow" style={{ minHeight: "400px" }}>
        ...
        >
          ...
          />
          {/* Show all vehicle markers for admin */}
          {vehicles && vehicles.length > 0 && vehicles.map((v, idx) => (
            v.location && v.location.latitude && v.location.longitude ? (
              <Marker key={v.id || idx} position={[v.location.latitude, v.location.longitude]} icon={carIcon}>
                <Popup>
                  Vehicle ID: {v.id || v.vehicleId}<br />
                  Driver: {v.driverName || v.driver_id || v.driverId || "N/A"}<br />
                  Lat: {v.location.latitude.toFixed(6)}, Lng: {v.location.longitude.toFixed(6)}
                </Popup>
              </Marker>
            ) : null
          ))}
          {/* Show employee and driver markers if present (for other dashboards) */}
          {employeeLatLng && (
